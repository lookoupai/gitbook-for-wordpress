...
function ensure_ai_service_available() {
    // 使用缓存避免频繁检查
    $cache_key = 'ai_service_registration_check';

    // 查看是否检查过
    $checked = get_transient($cache_key);
    if ($checked !== false) {
        return;
    }
...
// 只在管理后台初始化时检查一次，减少前台页面的检查
add_action('admin_init', 'ensure_ai_service_available');

// 移除wp_loaded钩子，减少频繁检查

// 检查AI Services插件是否已安装并激活
function is_ai_services_active() {
    // 使用transient缓存检查结果，减少重复检查
    $cache_key = 'ai_services_status_check';
    $force_check = isset($_GET['force_ai_check']) && current_user_can('administrator');
    $debug_mode = isset($_GET['debug_ai_services']) && current_user_can('administrator');
    ...
    if (!function_exists('ai_services')) {
        ...
        set_transient($cache_key, false, HOUR_IN_SECONDS); // 缓存1小时
        return false;
    }
    ...
    try {
        ...
        try {
            ...
            // 缓存结果以提高性能
            set_transient($cache_key, $has_services, HOUR_IN_SECONDS); // 缓存1小时
            return $has_services;
        } catch (Exception $e) {
            if ($debug_mode && current_user_can('administrator')) {
...
function render_ai_review_settings_page() {
    ...
    // 判断是否需要强制检查AI服务状态
    $force_check = isset($_GET['check_ai_services']);
    ...
    // 使用缓存获取AI服务状态 - 使用不同的缓存键避免冲突
    $cache_key = 'ai_services_settings_status';
    $ai_services_status = null;
    ...
    if ($force_check) {
        ...
        // 使用缓存的结果
        $ai_services_status = get_transient($cache_key);
    }

    // 如果没有缓存结果或强制检查，则使用新的详细检查函数
    if ($ai_services_status === null) {
        // 使用新的详细检查函数获取AI服务状态
        $ai_services_status = get_ai_services_detailed_status();

        // 缓存结果10分钟
        set_transient($cache_key, $ai_services_status, 10 * MINUTE_IN_SECONDS);

        if ($force_check) {
            error_log('渲染AI审核设置页面：完成检查AI服务状态，发现 ' . (is_array($ai_services_status) && isset($ai_services_status['available_services']) && is_array($ai_services_status['available_services']) ? count($ai_services_status['available_services']) : 0) . ' 个服务');
        }
    }
    ...
    if (is_array($ai_services_status)) {
        $has_ai_services = isset($ai_services_status['has_ai_services']) ? (bool)$ai_services_status['has_ai_services'] : false;
        $available_services = isset($ai_services_status['available_services']) && is_array($ai_services_status['available_services']) ? $ai_services_status['available_services'] : array();
    }
    ?>
    <div class="wrap">
        <h1>AI文章审核设置</h1>
        ...
        <div class="notice notice-warning">
            ...
            <p>
                ...
                <a href="<?php echo esc_url(add_query_arg('check_ai_services', '1')); ?>" class="button">重新检查AI服务状态</a>
            </p>
        </div>
        <?php else: ?>
        ...
        <form method="post" action="">
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        <select name="ai_review_service">
                            ...
                            <?php
                            // 使用前面收集的服务列表并进行类型检查
                            if (is_array($available_services) && !empty($available_services)):
                                foreach ($available_services as $service):
                                    if (is_array($service)):
                            ?>
                                <option value="<?php echo esc_attr(isset($service['name']) ? $service['name'] : ''); ?>" <?php selected($settings['service'], isset($service['name']) ? $service['name'] : ''); ?>>
                                    <?php echo esc_html(isset($service['label']) ? $service['label'] : '未命名服务'); ?>
                                </option>
                            <?php
                                    endif;
                                endforeach;
                ...
                <?php if (current_user_can('administrator') && isset($_GET['show_diagnostics'])): ?>
                <!-- 诊断信息区域 - 仅管理员可见 -->
                <tr>
                    <th scope="row">诊断信息</th>
                    <td>
                        <div class="ai-diagnostics-info" style="background: #f8f8f8; padding: 10px; border: 1px solid #ddd; max-height: 200px; overflow: auto;">
                            <h4>AI服务状态详情</h4>
                            <ul>
                                <li>AI Services插件已安装: <?php echo (is_array($ai_services_status) && isset($ai_services_status['details']) && isset($ai_services_status['details']['plugin_exists'])) ? ($ai_services_status['details']['plugin_exists'] ? '是' : '否') : '未知'; ?></li>
                                <li>ai_services()对象有效: <?php echo (is_array($ai_services_status) && isset($ai_services_status['details']) && isset($ai_services_status['details']['service_object_valid'])) ? ($ai_services_status['details']['service_object_valid'] ? '是' : '否') : '未知'; ?></li>
                                <li>has_available_services(): <?php echo (is_array($ai_services_status) && isset($ai_services_status['details']) && isset($ai_services_status['details']['has_available_services'])) ? ($ai_services_status['details']['has_available_services'] ? '是' : '否') : '未知'; ?></li>
                                <?php if (is_array($ai_services_status) && isset($ai_services_status['details']) && !empty($ai_services_status['details']['error'])): ?>
                                <li style="color: red;">错误信息: <?php echo esc_html($ai_services_status['details']['error']); ?></li>
                                <?php endif; ?>
                            </ul>

                            <h4>可用服务</h4>
                            <?php if (!is_array($ai_services_status) || !isset($ai_services_status['available_services']) || !is_array($ai_services_status['available_services']) || empty($ai_services_status['available_services'])): ?>
                            <p>未找到可用服务</p>
                            <?php else: ?>
                            <ul>
                                <?php foreach ($ai_services_status['available_services'] as $service): ?>
                                <?php if (is_array($service)): ?>
                                <li>名称: <?php echo esc_html(isset($service['name']) ? ($service['name'] ?: '(自动)') : '未知'); ?>, 标签: <?php echo esc_html(isset($service['label']) ? $service['label'] : '未知'); ?></li>
                                <?php endif; ?>
                                <?php endforeach; ?>
                            </ul>
                            <?php endif; ?>
                            ...
                            <h4>缓存信息</h4>
                            <ul>
                                <li>设置页面缓存键: <?php echo esc_html($cache_key); ?></li>
                                <li>状态检查缓存键: ai_services_status_check</li>
                                <li>服务注册检查缓存键: ai_service_registration_check</li>
                            </ul>
                        </div>
                        <p class="description">
                            <a href="<?php echo esc_url(add_query_arg(array('check_ai_services' => '1', 'show_diagnostics' => '1'))); ?>" class="button">刷新诊断信息</a>
                            <a href="<?php echo esc_url(remove_query_arg('show_diagnostics')); ?>" class="button">隐藏诊断信息</a>
                        </p>
                    </td>
                </tr>
                <?php else: ?>
                ...
                <tr>
                    <th scope="row">高级诊断</th>
                    <td>
                        <a href="<?php echo esc_url(add_query_arg('show_diagnostics', '1')); ?>" class="button">显示诊断信息</a>
                        <a href="<?php echo esc_url(admin_url('tools.php?page=ai-services-diagnostics')); ?>" class="button">AI服务诊断工具</a>
                    </td>
                </tr>
                <?php endif; ?>
                <?php endif; ?>
        ...
        do {
            ...
            try {
                ...
                if (!is_string($response_text)) {
                    ...
                    if (current_user_can('administrator')) {
                        error_log('已将非字符串响应强制转换为字符串，前100字符: ' . mb_substr($response_text, 0, 100, 'UTF-8'));
                    }
                }
                ...
                // 记录提取的文本
                if (current_user_can('administrator')) {
                        // 确保是字符串再使用mb_substr（多字节安全）
                        $log_text = is_string($response_text) ? $response_text : json_encode($response_text);
                        error_log('AI审核提取的文本: ' . mb_substr($log_text, 0, 200, 'UTF-8') .
                                 (mb_strlen($log_text, 'UTF-8') > 200 ? '...' : ''));
                        }
...
function extract_score_from_response($text, $debug = false) {
    ...
    if ($debug) {
        error_log(implode("\n", $log));
        error_log("原始文本：" . mb_substr($original_text, 0, 500, 'UTF-8') . (mb_strlen($original_text, 'UTF-8') > 500 ? "...(截断)" : ""));
    }
    return 0.7;
}
        ...
        try {
            ...
            do {
                ...
                if (!is_string($response_text)) {
                    ...
                    if (current_user_can('administrator')) {
                        error_log('已将修订审核非字符串响应强制转换为字符串，前100字符: ' . mb_substr($response_text, 0, 100, 'UTF-8'));
                    }
                }
                ...
                if (current_user_can('administrator')) {
                        // 确保是字符串再使用mb_substr（多字节安全）
                        $log_text = is_string($response_text) ? $response_text : json_encode($response_text);
                        error_log('AI审核修订提取的文本: ' . mb_substr($log_text, 0, 200, 'UTF-8') .
                                 (mb_strlen($log_text, 'UTF-8') > 200 ? '...' : ''));
                }
...
function trigger_ai_review_for_revision($revision_id) {
    ...
    // 如果父文章刚刚被审核并发布，跳过对这个修订版本的审核
    // 检查父文章是否刚刚被审核
    $parent_review_time = get_post_meta($parent_id, '_ai_review_date', true);
    if ($parent_review_time) {
        $parent_review_timestamp = strtotime($parent_review_time);
        // 如果父文章的审核时间和修订版本的创建时间非常接近（10秒内）
        // 说明这个修订版本可能是由于文章状态变化自动创建的
        if (abs($revision_time - $parent_review_timestamp) < 10) {
            if (current_user_can('administrator')) {
                error_log('跳过审核：该修订版本可能是由于文章状态变化自动创建的，与父文章审核时间相距 ' . abs($revision_time - $parent_review_timestamp) . ' 秒');
            }
            return;
        }
    }
    ...
    if ($parent_post->post_type !== 'post') {
        if (current_user_can('administrator')) {
            error_log('父文章不是post类型，跳过审核: ' . $parent_post->post_type);
        }
        return;
    }
    ...
    // 检查修订版本的内容是否与父文章的内容相同
    // 如果内容相同，说明这可能是由于状态变化自动创建的修订版本
    if ($revision->post_content === $parent_post->post_content && $revision->post_title === $parent_post->post_title) {
        if (current_user_can('administrator')) {
            error_log('跳过审核：修订版本的内容与父文章完全相同，可能是由于状态变化自动创建的');
        }
        return;
    }

    // 检查是否需要审核（任何实质性修改的版本都应该审核）
...
function extract_text_from_candidates($candidates, $debug = false) {
    ...
    try {
        ...
        if (is_object($candidates)) {
            ...
            if (!empty($text)) {
                $log[] = "通过递归查找成功获取text: " . mb_substr($text, 0, 50, 'UTF-8');
                if ($debug) error_log(implode("\n", $log));
                return $text;
            }
...
// 添加AI服务状态诊断工具
function ai_services_diagnostics() {
    if (!current_user_can('administrator')) {
        wp_die('您没有权限访问此页面');
    }

    // 清除缓存并强制进行新的检查
    delete_transient('ai_services_status_check');
    ...
    // 检查函数是否存在
    if (function_exists('ai_services')) {
        echo '<li style="color:green">✓ AI Services插件已安装和激活</li>';

        // 获取AI Services实例
        try {
            $ai_services = ai_services();
            if ($ai_services) {
                echo '<li style="color:green">✓ ai_services()返回有效对象</li>';

                // 检查用户权限
                if (current_user_can('ais_access_services')) {
                    echo '<li style="color:green">✓ 当前用户拥有AI Services访问权限</li>';
                } else {
                    echo '<li style="color:orange">! 当前用户没有显式的ais_access_services权限，但作为管理员可以访问</li>';
                }

                // 检查API方法
                if (method_exists($ai_services, 'has_available_services')) {
                    echo '<li style="color:green">✓ has_available_services方法存在</li>';

                    $has_services = $ai_services->has_available_services();
                    if ($has_services) {
                        echo '<li style="color:green">✓ 存在可用的AI服务</li>';

                        // 检查available_service方法
                        if (method_exists($ai_services, 'get_available_service')) {
                            $available_service = $ai_services->get_available_service();
                            if ($available_service) {
                                echo '<li style="color:green">✓ 成功获取可用服务</li>';

                                echo '<h2>服务详情</h2>';
                                echo '<ul>';

                                if (method_exists($available_service, 'get_name')) {
                                    echo '<li>服务名称: ' . esc_html($available_service->get_name()) . '</li>';
                                }

                                if (method_exists($available_service, 'get_label')) {
                                    echo '<li>服务标签: ' . esc_html($available_service->get_label()) . '</li>';
                                }

                                if (method_exists($available_service, 'get_models')) {
                                    $models = $available_service->get_models();
                                    echo '<li>可用模型数量: ' . count($models) . '</li>';

                                    if (!empty($models)) {
                                        echo '<li>可用模型:';
                                        echo '<ul>';
                                        foreach ($models as $model) {
                                            echo '<li>' . esc_html($model) . '</li>';
                                        }
                                        echo '</ul></li>';
                                    }
                                }
                                ...
                                if (method_exists($available_service, 'is_available')) {
                                    echo '<li>服务是否可用: ' . ($available_service->is_available() ? '是' : '否') . '</li>';
                                }
                                ...
                                // 添加测试区域
                                echo '<h2>服务测试</h2>';
                                echo '<p>您可以在这里测试AI服务是否正常工作</p>';
                                echo '<form method="post" action="">';
                                echo '<input type="hidden" name="test_ai_service" value="1">';
                                echo '<textarea name="test_text" rows="4" cols="50" placeholder="输入要测试的文本...">这是一个测试文本，请AI评估这段内容的质量，并给出1-10分的评分。</textarea><br>';
                                echo '<input type="submit" class="button button-primary" value="测试AI服务">';
                                echo '</form>';

                                // 处理测试请求
                                if (isset($_POST['test_ai_service'])) {
                                    $test_text = isset($_POST['test_text']) ? sanitize_textarea_field($_POST['test_text']) : '这是一个测试文本';
                                    echo '<h3>测试结果</h3>';

                                    try {
                                        // 构建测试请求
                                        $test_result = test_ai_service($test_text);

                                        if (is_wp_error($test_result)) {
                                            echo '<div style="color:red"><strong>测试失败:</strong> ' . esc_html($test_result->get_error_message()) . '</div>';
                                        } else {
                                            echo '<div style="color:green"><strong>测试成功!</strong></div>';
                                            echo '<pre>' . esc_html(print_r($test_result, true)) . '</pre>';
                                        }
                                    } catch (Exception $e) {
                                        echo '<div style="color:red"><strong>测试出错:</strong> ' . esc_html($e->getMessage()) . '</div>';
                                    }
                                }

                            } else {
                                echo '<li style="color:red">✗ get_available_service()返回null</li>';
                            }
                        } else {
                            echo '<li style="color:red">✗ get_available_service方法不存在</li>';
                        }
                    } else {
                        echo '<li style="color:red">✗ 没有可用的AI服务，请配置AI Service插件</li>';
                    }
                } else {
                    echo '<li style="color:red">✗ has_available_services方法不存在</li>';
                }
            } else {
                echo '<li style="color:red">✗ ai_services()返回无效对象</li>';
            }
        } catch (Exception $e) {
            echo '<li style="color:red">✗ 获取ai_services()实例时出错: ' . esc_html($e->getMessage()) . '</li>';
        }
    } else {
        echo '<li style="color:red">✗ AI Services插件未安装或未激活</li>';
    }

    echo '</ul>';

    // 缓存状态信息
    $cache_status = get_transient('ai_services_status_check');
    echo '<h2>缓存状态</h2>';
    echo '<p>当前缓存值: ' . ($cache_status ? '有效 (true)' : '无效或不存在 (false)') . '</p>';

    // 添加刷新和设置链接
    echo '<p><a href="' . esc_url(add_query_arg('force_check', '1')) . '" class="button button-primary">重新检查</a> ';
    echo '<a href="' . esc_url(admin_url('options-general.php?page=ai-services-settings')) . '" class="button">配置AI服务</a>';
    echo '<a href="' . esc_url(admin_url('options-general.php?page=ai-review-settings')) . '" class="button">AI审核设置</a></p>';

    echo '</div>';
}

// 添加诊断页面到管理菜单
function register_ai_services_diagnostics_page() {
    add_submenu_page(
        'tools.php',
        'AI服务诊断',
        'AI服务诊断',
...
add_action('admin_menu', 'register_ai_services_diagnostics_page');

// 获取详细的AI服务状态信息，专用于设置页面
function get_ai_services_detailed_status() {
    $result = array(
        'has_ai_services' => false,
        'available_services' => array(),
        'details' => array(
            'plugin_exists' => function_exists('ai_services'),
            'service_object_valid' => false,
    ...
    if (!function_exists('ai_services')) {
        $result['details']['error'] = 'AI Services插件未安装或未激活';
        return $result;
    }
    ...
    try {
        $ai_services = ai_services();
        if (!$ai_services) {
            $result['details']['error'] = 'ai_services()返回无效对象';
            return $result;
        }

        $result['details']['service_object_valid'] = true;

        if (!method_exists($ai_services, 'has_available_services')) {
            $result['details']['error'] = 'ai_services对象没有has_available_services方法';
            return $result;
        }
        ...
        try {
        $has_services = $ai_services->has_available_services();
            $result['has_ai_services'] = (bool)$has_services;
            $result['details']['has_available_services'] = (bool)$has_services;
        } catch (Exception $e) {
            $result['details']['error'] = '检查服务可用性出错: ' . $e->getMessage();
            $result['has_ai_services'] = false;
            $result['details']['has_available_services'] = false;
            return $result;
        }

        // 初始化可用服务数组
        $result['available_services'] = array();

        if ($has_services && method_exists($ai_services, 'get_available_service')) {
            try {
            $available_service = $ai_services->get_available_service();
            ...
            if ($available_service) {
                ...
                // 获取服务信息出错，但不影响主要状态检查
                $result['details']['error'] = '获取服务信息出错: ' . $e->getMessage();
            }
        }

        // 如果没有获取到服务，但has_available_services返回true，添加默认服务
        if (empty($result['available_services']) && $has_services) {
            $result['available_services'][] = array(
                'name' => 'auto_select',  // 使用非空名称
                'label' => '自动选择AI服务'
            );
        }

    } catch (Exception $e) {
        $result['details']['error'] = 'AI服务检查出错: ' . $e->getMessage();
    }

    // 确保永远返回正确的数据结构
    if (!isset($result['has_ai_services'])) {
        $result['has_ai_services'] = false;
    }

    if (!isset($result['available_services']) || !is_array($result['available_services'])) {
        $result['available_services'] = array();
    }
